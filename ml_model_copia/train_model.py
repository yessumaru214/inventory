...
if len(sys.argv) != 7:
    error_response = {
        "status": "error",
        "message": "Se requieren 6 argumentos: <ruta_X> <ruta_y> <max_depth> <n_estimators> <min_samples_split> <min_samples_leaf>"
    }
...
try:
    ...
    mad = sum(abs(y_pred - y_test)) / len(y_test)  # Cálculo del MAD
...
success_response = {
    ...
    "MSE": mse,
    "MAPE": mape,
    "MAD": mad,  # Agregar el MAD a la respuesta
